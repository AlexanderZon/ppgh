...
<script type="text/javascript">  
    $(document).ready(function() {	
        $("#seguranca_from_date").datepicker({   
            defaultDate: "+1w",
            dateFormat: 'dd-mm-yy',        
            changeMonth: true,
            changeYear: true,
            onClose: function( selectedDate ) {
                $( "#seguranca_to_date" ).datepicker( "option", "minDate", selectedDate );
            }
        });    
        $("#seguranca_to_date").datepicker({
            defaultDate: "+1w",
            dateFormat: 'dd-mm-yy',          
            changeMonth: true,
            changeYear: true,
            onClose: function( selectedDate ) {
                $( "#seguranca_from_date" ).datepicker( "option", "maxDate", selectedDate );
                if(!$("#seguranca_from_date").val())
                    {
                        $( "#seguranca_from_date" ).val($("#seguranca_to_date").val());
                    }
            }
        });  
        $("#busca").click(function() {
        
            if($("#seguranca_from_date").val() && !$("#seguranca_to_date").val() )
            {
                alert("Data de fim Obrigatoria");
                return false;
            }
            $("#frmChk").submit();
...
<?php endif; ?>
<div id="renglon" style="margin-bottom: 0px; ">
    <?php echo form_tag('seguranca/index',array('name' => 'frmChk', 'id' => 'frmChk','style'=>'margin:0px')) ?>
    <div class="propiedades propiedades-extend" style="width: 840px; height: 80px;">
        <h1 style="border: 0px !important; font-size: 12px;"><?php echo __('Consulta de Pessoas') ?></h1>
        <table width="80%" >
            <tr>
...
<?php if ($LogActividadess->haveToPaginate()): ?>
    <div class="result_busca" style="margin-top: 33px;">
  ...
  <?php if ($LogActividadess->getNbResults()): ?>
    ...
    <tr class="<?php echo $class;?>" valign="top" onmouseover="javascript:overRow(<?php echo $i; ?>);" onmouseout="javascript:outRow(<?php echo $i; ?>);">
        <td class="borderBottomDarkGray" width="28" align="center">
            <!--&nbsp;<input type="checkbox" id="chk_<?php //echo $LogActividades->getIdLog() ?>" name="chk[<?php //echo $LogActividades->getIdLog() ?>]" value="<?php //echo $LogActividades->getIdLog() ?>">&nbsp;-->
        </td>
