    ...
    if($request->isMethod('post'))
    {
        if($this->getRequestParameter('from_date') > $this->getRequestParameter('to_date') && $this->getRequestParameter('to_date'))
        {
            ...
            if($this->getRequestParameter('from_date'))
            {
                if($this->getRequestParameter('from_date'))
                {
                    $inicio   = $fecha->formatoFechaMySQL($this->getRequestParameter('from_date'));
                    $fim      = $fecha->formatoFechaMySQL($this->getRequestParameter('to_date'));
                }
                $cFecha = $c->getNewCriterion(LogActividadesPeer::FECHA, $inicio,Criteria::GREATER_EQUAL);
                $cFecha->addAnd($c->getNewCriterion(LogActividadesPeer::FECHA, $fim, Criteria::LESS_EQUAL));
                $c->add($cFecha);
                $c->addAscendingOrderByColumn(LogActividadesPeer::FECHA);
            }
