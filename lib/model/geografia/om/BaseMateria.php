...
abstract class BaseMateria extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the materia field.
	 * @var        string
	 */
	protected $materia;
	...
	public function setIdMateria($v)
	{
		...
		if ($this->id_materia !== $v) {
			$this->id_materia = $v;
			$this->modifiedColumns[] = MateriaPeer::ID_MATERIA;
		}
	...
	/**
	 * Set the value of [materia] column.
	 * 
	...
	public function setMateria($v)
	{
		...
		if ($this->materia !== $v) {
			$this->materia = $v;
			$this->modifiedColumns[] = MateriaPeer::MATERIA;
		}
	...
	public function setStatus($v)
	{
		...
		if ($this->status !== $v) {
			$this->status = $v;
			$this->modifiedColumns[] = MateriaPeer::STATUS;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->id_materia = (isset($row[$startcol + 0]) && $row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->materia = (isset($row[$startcol + 1]) && $row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			...
			return $startcol + 3; // 3 = MateriaPeer::NUM_COLUMNS - MateriaPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$this->setIdMateria($pk);  //[IMV] update autoincrement primary key
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			$keys[0] => $this->getIdMateria(),
			$keys[1] => $this->getMateria(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setIdMateria($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setMateria($arr[$keys[1]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(MateriaPeer::ID_MATERIA)) $criteria->add(MateriaPeer::ID_MATERIA, $this->id_materia);
		if ($this->isColumnModified(MateriaPeer::MATERIA)) $criteria->add(MateriaPeer::MATERIA, $this->materia);
		if ($this->isColumnModified(MateriaPeer::STATUS)) $criteria->add(MateriaPeer::STATUS, $this->status);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(MateriaPeer::ID_MATERIA, $this->id_materia);
	...
	/**
	 * Generic method to set the primary key (id_materia column).
	 *
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setMateria($this->materia);
		...
		$copyObj->setIdMateria(NULL); // this is a auto-increment column, so set to default value
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseMateria:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseMateria::%s', $method));
	  }
