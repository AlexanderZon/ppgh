...
class SfSectionI18nPeer extends BaseSfSectionI18nPeer {
    ...
    public static function displaySectionsTree($id_padre, $id_profile)
    {
            ...
            $c->add(SfSectionPeer::STATUS,'1', Criteria::EQUAL);
    ...
    public static function getBuscaSecciones($lang, $busca)
    {
        $c = new Criteria();
        //Eliminamos la columnas de seleccion en caso de que esten definidas
        $c->clearSelectColumns();
        ...
        $c->addSelectColumn(self::NAME_SECTION);
        $c->addSelectColumn(self::DESCRIP_SECTION);
        //Filtros
        if($busca)
        {
            $criterio = $c->getNewCriterion(self::NAME_SECTION, '%'.$busca.'%', Criteria::LIKE);
            $criterio->addOr($c->getNewCriterion(self::DESCRIP_SECTION, '%'.$busca.'%', Criteria::LIKE));
            $c->add($criterio);
        }
        $c->addJoin(self::ID, SfSectionPeer::ID, Criteria::INNER_JOIN);
        ...
        while($res = $rs->fetch())
        {
            $dato['link'] = $res['SW_MENU'];
            $dato['nome'] = $res['NAME_SECTION'];
            $dato['des'] = $res['DESCRIP_SECTION'];
            $datos[] = $dato;
        }
        if (!empty($datos)){
            return $datos;
