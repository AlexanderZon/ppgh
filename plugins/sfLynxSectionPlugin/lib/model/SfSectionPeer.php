...
class SfSectionPeer extends BaseSfSectionPeer {
    ...
    public static function getPrincipalSection() {
        $c = new Criteria();
    ...
    /**
     * Devuelve los datos de una seccion segun el permalink que recibe.
     * Si la seccion no existe, devuelve false.
     *
     * @param String $swMenu Enlace permalink de una seccion
     */
    public static function validateSwMenu($swMenu, $culture = null) {
        $c = new Criteria();
        //Eliminamos la columnas de seleccion en caso de que esten definidas
        $c->clearSelectColumns();
        //Se Agregan las Columnas necesarias
        ...
        //Filtros
        $c->add(self::SW_MENU ,$swMenu, Criteria::EQUAL);
        $c->addJoin(self::ID,SfSectionI18nPeer::ID,Criteria::INNER_JOIN);
        if($culture) {
          $c->add(SfSectionI18nPeer::LANGUAGE, $culture, Criteria::EQUAL);
        }
    ...
    /**
     * Devuelve todos los datos para la seccion $swMenu con el idioma
     * pasado $language.
     *
     * @param String $swMenu   Enlace permalink de una seccion
     * @param String $language Idioma
    */
    public static function retrieveSwMenuByLang($swMenu, $language) {
       $c = new Criteria();
       //Eliminamos la columnas de seleccion en caso de que esten definidas
       $c->clearSelectColumns();
       //Se Agregan las Columnas necesarias
       $c->addSelectColumn(self::SW_MENU);
       $c->addSelectColumn(self::SPECIAL_PAGE);
       $c->addSelectColumn(self::SHOW_TEXT);
       $c->addSelectColumn(self::ONLY_COMPLEMENT);
       $c->addSelectColumn(SfSectionI18nPeer::NAME_SECTION);
       ...
       $c->addJoin(self::ID,SfSectionI18nPeer::ID,Criteria::INNER_JOIN);
       $c->add(self::SW_MENU,$swMenu,Criteria::EQUAL);
       $c->add(SfSectionI18nPeer::LANGUAGE,$language,Criteria::EQUAL);
       $c1 = $c->getNewCriterion(self::STATUS,1,Criteria::EQUAL);
       $c2 = $c->getNewCriterion(self::STATUS,2,Criteria::EQUAL);
       //$c3 Agregado para soportar el status "enable in upper menu"
       $c3 = $c->getNewCriterion(self::STATUS,3,Criteria::EQUAL);
       $c1->addOr($c2);
       $c1->addOr($c3);
       $c->add($c1);

       $data = SfSectionI18nPeer::doSelectStmt($c);
       if(!$data->rowCount()) {
         return false;
       }else {
     ...
     public static function getIdSectionBySecoe($seccoe)
     {
         $c = new Criteria();
         $c->add(self::SW_MENU, $seccoe, Criteria::EQUAL);
         return self::doSelectOne($c);
     }
     
     public static function displaySections($id_padre)
     {
      ...
      {
        $datoSeccion['id'] = $res['ID'];
        $datoSeccion['name_sw_menu'] = $res['SW_MENU'];
        $datoSeccion['position'] = $res['POSITION'];
        $datos[] = $datoSeccion;
       }
       if (!empty($datos)){
