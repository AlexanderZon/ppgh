...
class lxsectionActions extends sfActions
{
    ...
    public function executeEditContent(sfWebRequest $request)
    {
        ...
        $this->sf_section_i18n = new SfSectionI18nForm();
    ...
    {
        $section = SfSectionPeer::retrieveByPK($request->getParameter('id'));  
        $SfSectionI18n = SfSectionI18nPeer::retrieveByPk($request->getParameter('id'),$request->getParameter('language'));
        if(!$SfSectionI18n)
        {
            $this->section = new SfSectionI18n();
            $this->section->setId($request->getParameter('id'));
            $this->section->setLanguage($request->getParameter('language'));
            $this->section->setNameSection($request->getParameter('nome_seccao'));
            $this->section->setMetaTitle($request->getParameter('meta_title'));
            $this->section->setMetaDescription($request->getParameter('meta_des'));
            $this->section->setMetaKeyword($request->getParameter('meta_key'));
            $this->section->setDescripSection($request->getParameter('descripcion'));
            $this->section->save();
        }else{
            $SfSectionI18n->setNameSection($request->getParameter('nome_seccao'));
            $SfSectionI18n->setMetaTitle($request->getParameter('meta_title'));
            $SfSectionI18n->setMetaDescription($request->getParameter('meta_des'));
            $SfSectionI18n->setMetaKeyword($request->getParameter('meta_key'));
            $SfSectionI18n->setDescripSection($request->getParameter('descripcion'));
            $SfSectionI18n->save();
        }  
        echo '1';
        return sfView::NONE;    
	...
	$result = SfSectionI18nPeer::displaySectionsTree($node, $this->getUser()->getAttribute('idProfile'));
	$languagePpal = SfLanguagePeer::getLanguagePrincipal();
	// display each child
        if($result){
	...
	{
            $result = SfSectionPeer::displaySections($padre_antes);
            $total = count($result);
    ...
    /**
     * Construye las pestañas con las culturas activas. Obtiene los datos de la
     * tabla de idiomas
     */
    public function executeGetTabs(sfWebRequest $request)
    {
        $this->forward404Unless($request->isXmlHttpRequest());

        $c = new Criteria();
        $c->addAscendingOrderByColumn(SfLanguagePeer::LANGUAGE);
        $c->addAscendingOrderByColumn(SfLanguagePeer::COUNTRY);
        $c->addDescendingOrderByColumn(SfLanguagePeer::PRINCIPAL);
        $this->cultures     = SfLanguagePeer::doSelect($c);
        $this->id_section = $request->getParameter('id_section');

        $this->setLayout(false);
    }
    
    public function executeListaBanners(sfWebRequest $request)
    {
        //$this->forward404Unless($request->isXmlHttpRequest());
        $c = new Criteria();
        $this->listbanners = BannerI18nPeer::doSelect($c);
    }
    
    public function executeSelectBanner(sfWebRequest $request)
    {
        $this->id = $request->getParameter('id_banner'); 
        $this->banner = BannerI18nPeer::retrieveByPK($this->id);
    }
}
