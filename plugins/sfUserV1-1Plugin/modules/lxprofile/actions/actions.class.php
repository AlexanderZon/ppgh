 ...
 *
 * @package    lynx4
 * @subpackage lxprofile
 * @author     Henry Vallenilla <henryvallenilla@gmail.com>
 */
class lxprofileActions extends sfActions
{
  public function executeIndex(sfWebRequest $request)
  {
  
      $this->getResponse()->setTitle($this->getContext()->getI18N()->__('Lista Núcleos').'  - '.sfConfig::get('app_name_app'));
	if (!$this->getRequestParameter('buscador')){
		...
		if(!$this->getRequestParameter('by'))
		{
            ...
            $this->sort = 'name_profile';      // Nombre del campo que por defecto se ordenara
		}
		...
		if($this->getRequestParameter('sort'))
		{
			...
			switch ($this->getRequestParameter('by')) {
				...
				case 'desc':
					$c->addDescendingOrderByColumn(LxProfilePeer::$this->getRequestParameter('sort'));
					$this->by = "asc";
					$this->by_page = "desc";
					break;
				default:
					$c->addAscendingOrderByColumn(LxProfilePeer::$this->getRequestParameter('sort'));
					$this->by = "desc";
					$this->by_page = "asc";
					break;
			}
		...
		{
                //PERSONALIZAR SEGUN LA NECESIDAD DEL MODULO
            $criterio = $c->getNewCriterion(LxProfilePeer::ID_PROFILE, '%'.$this->getRequestParameter('buscador').'%', Criteria::LIKE);
                $criterio->addOr($c->getNewCriterion(LxProfilePeer::NAME_PROFILE, '%'.$this->getRequestParameter('buscador').'%', Criteria::LIKE));
                $criterio->addOr($c->getNewCriterion(LxProfilePeer::STATUS, '%'.$this->getRequestParameter('buscador').'%', Criteria::LIKE));
                $c->add($criterio);
		...
		$pager = new sfPropelPager('LxProfile',20);
		$pager->setCriteria($c);
		$pager->setPage($this->getRequestParameter('page',1));
		$pager->init();
		$this->LxProfiles = $pager;                
        // Crea sesion de la uri al momento
        $this->getUser()->setAttribute('uri_lxprofile','sort='.$this->sort.'&by='.$this->by_page.$buscador.'&page='.$this->LxProfiles->getPage());
  ...
  public function executePermission(sfWebRequest $request)
  {
    ...
    $this->LxPrivileges = LxPrivilegePeer::getAllPrivileges();    
   ...
   * @return <NONE>
   */
  public function executeChangePrivileges(sfWebRequest $request)
  {
    $this->forward404If($this->getUser()->getAttribute('idProfile')==$request->getParameter('id_profile') or $request->getParameter('id_profile') == 1);
    $this->setLayout(false);
    if($request->getParameter('status'))
    {
        //Debo registrar los permisos
        if(!LxProfileModulePeer::valPrivilege($request->getParameter('id_privilege'), $request->getParameter('id_profile'), $request->getParameter('id_module')))
        {
            if($request->getParameter('privPpal'))
            {
                // Si no existe, entonces lo registro
                if(!LxProfileModulePeer::valPrivilege($request->getParameter('privPpal'), $request->getParameter('id_profile'), $request->getParameter('id_module'))){
                    // Registro el privilegio principal
                    LxProfileModulePeer::newPermission($request->getParameter('privPpal'), $request->getParameter('id_profile'), $request->getParameter('id_module'));
                }
            }
            // Registro el privilegio seleccionado
            LxProfileModulePeer::newPermission($request->getParameter('id_privilege'), $request->getParameter('id_profile'), $request->getParameter('id_module'));
        }         
    }else{
        /**
        ...
        {            
            LxProfileModulePeer::deleteAllPermissions($request->getParameter('id_profile'), $request->getParameter('id_module'));
        }else{
            /* Es otro privilegio que no tiene dependencia jerarquica*/
            LxProfileModulePeer::deletePermission($request->getParameter('id_privilege'), $request->getParameter('id_profile'), $request->getParameter('id_module'));
        }          
    }    
    return sfView::NONE;
  }
