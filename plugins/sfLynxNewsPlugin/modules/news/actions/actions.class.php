...
class newsActions extends sfActions
{
  ...
  public function executeIndex(sfWebRequest $request)
  {
    ...
    if (!$this->getRequestParameter('buscador')){
            ...
            $this->sort = 'date';      // Nombre del campo que por defecto se ordenara
            }
  ...
  public function executeNew(sfWebRequest $request)
  {
    ...
    $news->setStatus(2);
    $news->save();
    $newsI18n =  new SfNewsI18n();
    $newsI18n->setId($news->getIdNews());
    $newsI18n->setTitle('');
    $newsI18n->setLanguage('pt');
    $newsI18n->save();
    $this->redirect('news/edit?id_news='.$news->getIdNews().'&language=pt');
  }
  ...
  {
    $news = SfNewsPeer::retrieveByPK($request->getParameter('id'));  
    $news->setStatus('1');
    $NewsI18n = SfNewsI18nPeer::retrieveByPk($request->getParameter('id'),$request->getParameter('language'));
    if(!$NewsI18n)
    {
        $this->news = new SfNewsI18n();
        $this->news->setId($request->getParameter('id'));
        $this->news->setLanguage($request->getParameter('language'));
        $this->news->setTitle($request->getParameter('titulo'));
        $this->news->setSummary($request->getParameter('resumen'));
        $this->news->setBody($request->getParameter('descripcion'));
        $this->news->setPermalink(globalFunctions::crearPermalink($request->getParameter('titulo')));
        $this->news->save();
    }else{
        $NewsI18n->setTitle($request->getParameter('titulo'));
        $NewsI18n->setSummary($request->getParameter('resumen'));
        $NewsI18n->setBody($request->getParameter('descripcion'));
        $NewsI18n->setPermalink(globalFunctions::crearPermalink($request->getParameter('titulo')));
        $NewsI18n->save();
    }  
    echo '1';
    return sfView::NONE;    
  ...
  public function executeDelete(sfWebRequest $request)
  {
    ...
    $SfNews->delete();
    SfNewsI18nPeer::deleita($request->getParameter('id_news'));
